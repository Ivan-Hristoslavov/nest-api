...
export class CompetitorsAlertsBilling1786963171701 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."competitors_scrape_status_enum" AS ENUM('pending', 'success', 'failed', 'skipped')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_competitors_active_last_checked" ON "competitors" ("is_active", "last_checked_at") `,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."alerts_type_enum" AS ENUM('price_drop', 'price_rise', 'undercut', 'all_time_low', 'out_of_stock', 'scrape_failing')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."alerts_severity_enum" AS ENUM('info', 'warning', 'critical')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."alerts_delivery_status_enum" AS ENUM('pending', 'delivered', 'failed', 'skipped')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE INDEX "idx_alerts_unacknowledged" ON "alerts" ("acknowledged_at") `,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_alerts_product_created" ON "alerts" ("product_id", "created_at") `,
    );
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."users_status_enum" AS ENUM('pending', 'active', 'expired', 'suspended')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."users_plan_enum" AS ENUM('free', 'starter', 'pro', 'business')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "products" ADD "competitor_count" integer NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(`ALTER TABLE "products" ADD "our_price" numeric(12,2)`);
    await queryRunner.query(
      `CREATE INDEX "idx_price_history_competitor_recorded" ON "price_history" ("competitor_id", "recorded_at") `,
    );
    await queryRunner.query(
      `ALTER TABLE "price_history" ADD CONSTRAINT "FK_52cb611fd8151a893dd82b996df" FOREIGN KEY ("competitor_id") REFERENCES "competitors"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "competitors" ADD CONSTRAINT "FK_c68675be426f567a0a2c5801ab5" FOREIGN KEY ("product_id") REFERENCES "products"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD CONSTRAINT "FK_137b233903b939bcae8421722ee" FOREIGN KEY ("product_id") REFERENCES "products"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" ADD CONSTRAINT "FK_da354081829e982f6b8f0925aa4" FOREIGN KEY ("competitor_id") REFERENCES "competitors"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "alerts" DROP CONSTRAINT "FK_da354081829e982f6b8f0925aa4"`,
    );
    await queryRunner.query(
      `ALTER TABLE "alerts" DROP CONSTRAINT "FK_137b233903b939bcae8421722ee"`,
    );
    await queryRunner.query(
      `ALTER TABLE "competitors" DROP CONSTRAINT "FK_c68675be426f567a0a2c5801ab5"`,
    );
    await queryRunner.query(
      `ALTER TABLE "price_history" DROP CONSTRAINT "FK_52cb611fd8151a893dd82b996df"`,
    );
